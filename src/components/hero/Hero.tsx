 ...
 * title (masked reveal) → role → intro → CTAs → socials → terminal —
...
export function Hero() {
  ...
  // essentially finished — the last beat of the sequence. Under reduced motion
  // there is no sequence to wait for, so that case is derived rather than set
  // from the effect body (which would be a cascading render).
  const [settled, setSettled] = useState(false);
  const terminalLive = settled || (booted && reduced);
  useEffect(() => {
    if (!booted || reduced) return;
    const id = window.setTimeout(() => setSettled(true), T.settle * 1000);
  ...
  //
  // Measured in raw pixels off the *page* scroll rather than as a progress
  // ratio through this section. `useScroll({ target })` has to measure the
  // element, and under Lenis those measurements came back unstable here — the
  // ratio would climb, then fall back toward zero, so the cue faded out and
  // then reappeared straight over the navbar. Page scroll needs no measurement
  // and the hero is exactly one viewport tall, so a viewport-relative distance
  ...
  // Only the pointer/scroll displacement lives here. `heroFade` deliberately
  // does *not*: bound to `style.opacity` on these columns it never rendered,
  // because they sit inside the entrance variant tree, whose animations own
  // `opacity` and keep resetting it to 1. The fade now rides its own wrapper
  // above that tree (see below), where nothing else claims the property.
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <motion.div
            style={reduced ? undefined : { y: cueShift }}
            className="flex flex-col items-center gap-2"
          >
